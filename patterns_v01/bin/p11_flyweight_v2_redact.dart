...
class TransportTypeFactory {
  ...
  TransportType getTransportType({required String transportTypeName}) {
    ...
    TransportType newTransportTypeForAdding = TransportType(
      model3D: CadModel3D(name3D: transportTypeName),
    );

    addTransportType(newTransportType: newTransportTypeForAdding);
    print(
        '\tЗапрашиваемый вид транспорта - "$transportTypeName" не был найден.');
    print('\tНовый тип транспорта "$transportTypeName" добавлен.\n');
    return newTransportTypeForAdding;
    //------------------------------------------------------------
  }
